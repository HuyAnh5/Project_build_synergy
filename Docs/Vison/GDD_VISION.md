...
> 
> File này **không phải** bản implementation spec, không phải bản code handoff, và không thay thế các tài liệu chi tiết như `COMBAT_CORE_SPEC.md`, `SKILLS_PASSIVES_SPEC.md`, `PROJECT_META.md` hoặc các file spec chuyên trách.
> 
...
Đây là game roguelike turn-based với trọng tâm là **dice**, **skill slot**, **relic**, **status/payoff**, và **sequencing**. Người chơi không rút bài hay phản xạ thời gian thực; thay vào đó, đầu Player Phase dice tự roll, người chơi đọc tình huống, reorder dice nếu muốn, rồi click/drag skill vào target để cast ngay.
...
- mỗi lượt ngắn nhưng có ý nghĩa vì phải quyết định **reorder dice thế nào, skill nào dùng trước, target nào trước, setup trước hay nổ trước**,
...
- hành động được thực hiện qua **skill slot cố định + dice sequencing**, không phải chỉ là chọn skill rồi bấm vô thức,
...
- **Balatro**: relic kiểu joker, build engine, consumable bẻ hướng run, endless mode, unlock discovery.
...
Trong đó Player Phase đã bao gồm việc dice tự roll ngay đầu phase.

### 1. Player Phase — dice tự roll

Khi bắt đầu Player Phase, dice tự roll. Người chơi đọc kết quả roll, Crit/Fail, Base Value và các blue value có thể bị tác động.

### 2. Reorder nếu cần

Người chơi có thể reorder dice để quyết định skill tiếp theo sẽ consume dice nào. Reorder là lớp sequencing chính của combat hiện tại.

### 3. Click hoặc drag skill vào target để cast ngay

Người chơi bấm chọn skill rồi chọn target, hoặc kéo skill icon vào target. Nếu hợp lệ, skill cast ngay và consume dice theo slot cost từ trái sang phải theo thứ tự dice hiện tại.

### 4. Dice used state

Dice đã dùng không biến mất. Nó chuyển sang trạng thái used: hạ nhẹ trục Y và đổi background. Dice đó vẫn đọc được trên UI nhưng không còn available cho cast tiếp theo, trừ khi được refresh bởi turn mới hoặc consumable/effect.

### 5. End Phase và Enemy Phase

Khi không muốn dùng thêm skill, người chơi bấm End Turn để sang End Phase, sau đó đến Enemy Phase. Enemy hành động, cleanup diễn ra, rồi combat quay lại Player Phase nếu trận chưa kết thúc.

Hành động được resolve ngay khi cast hợp lệ. Player không cần chuẩn bị một hàng hành động riêng trước khi cho lượt chạy.
...
Người chơi đánh trận, nhận thưởng, chỉnh skill / dice / relic / consumable, rồi tiếp tục vào trận khó hơn với một build ngày càng rõ cá tính hơn.
...
Khi thua run, người chơi mất toàn bộ tiến trình của run hiện tại: dice, skill, relic, consumable và các tăng trưởng chỉ dành cho run đó. Thứ được giữ lại là **unlock progress**.
...
Game này phải mạnh ở cảm giác “chuẩn bị rồi kích nổ”. Người chơi setup Burn, Mark, Bleed, Freeze/Chilled, Guard break, AP state, dice order hoặc target state để rồi chuyển chúng thành payoff rõ ràng ở đúng thời điểm.
...
Status, lane, guard, consume, exact value, consumable, relic và dice customization phải giữ được cá tính riêng. Không để mọi hệ cuối cùng chỉ là cách khác nhau để stack cùng một loại burst vô danh.
...
- 4 skill slot
- Relic collection: có thể nhặt nhiều relic trong run, hiển thị ở góc trên UI giống STS-like relic bar
- 1 đến 3 dice
- Basic Attack và Basic Guard luôn tồn tại
- Consumables chia thành Fate / Seal / Rune

### Ý nghĩa định hướng

- số lượng công cụ phải đủ ít để người chơi đọc turn nhanh,
- nhưng mỗi công cụ phải đủ giàu tương tác để build có chiều sâu,
- basic actions tồn tại như anchor của economy, đảm bảo player luôn có một cách chơi hợp lệ khi roll xấu hoặc thiếu AP,
- số dice equip phải có ý nghĩa thật với action economy, không chỉ là cosmetic progression.

## 3. AP Economy

AP là tài nguyên nhịp độ. Nó không chỉ giới hạn số skill mạnh mà còn quyết định player có chọn setup, payoff hay basic action trong turn hiện tại.

AP phải luôn tạo cảm giác:

- thiếu vừa đủ để buộc lựa chọn,
- có thể xoay xở bằng build hoặc sequencing tốt,
- không trở thành nút thắt khiến combat tê liệt.

Các skill, relic, basic attack và một số payoff phải tương tác với AP sao cho người chơi cảm thấy có thể **xoay lượt**, chứ không chỉ chờ đủ AP rồi bấm kỹ năng mạnh nhất.
...
Nếu dice sequencing và thứ tự dùng skill mất ý nghĩa chiến thuật, game sẽ mất một nửa chiều sâu hiện có.

## 5. Element / Status / Payoff Engine

### Element payoff current rule

Element payoff hiện tại là **skill-text driven**. Không còn luật nền kiểu “Fire hit Burn thì auto consume Burn” hoặc “Ice hit Freeze/Chilled thì auto nhận AP/Guard”. Skill nào tác động vào status nào phải ghi rõ trong text. Ở scope hiện tại, mỗi hệ chủ yếu tương tác với status của chính hệ đó; cross-element reaction là future design space, chưa phải locked rule.
...
Relic trong game hiện là **passive-effect collection**: player có thể nhặt nhiều relic trong một run, relic hiển thị ở góc trên UI theo hướng giống relic bar của Slay the Spire, và từng relic phải có hiệu ứng nhỏ/hẹp hơn passive cũ vì chúng có thể cộng dồn.

Consumable là hệ riêng, không được gọi là Relic. Consumable chia thành **Fate / Seal / Rune**:

- **Fate**: can thiệp dice / roll / reload / reroll / face manipulation.
- **Seal**: combat direct aid / damage / status / cứu nguy.
- **Rune**: utility / buff / resource support.

### Relic pool là build pressure

Relic pool lớn là dụng ý thiết kế, không phải lỗi. Player không nên luôn tìm được đúng relic mình muốn, nhưng mỗi relic phải đủ rõ vai trò để khi nhặt được nó có thể đẩy build theo một hướng cụ thể.

### Consumable là công cụ bẻ nhịp ngắn hạn

Fate / Seal / Rune dùng để can thiệp một lượt, một tình huống combat, hoặc một quyết định dice/loadout ngắn hạn. Consumable không thay thế vai trò dài hạn của Relic.
...
2. **Skill slot + dice sequencing system**
3. **Relic build engine**
...
- **mở rộng công cụ**: skill, dice, relic, consumable,
...
6. Nó có giữ được giá trị của **dice order / sequencing** không?
...
# X. ĐỊNH HƯỚNG CHO SKILL, RELIC VÀ BUILD

## 1. Lenticular design là bắt buộc

Game không nên đi theo hướng 1 skill = 1 build biệt lập. Thay vào đó, skill và relic phải được thiết kế để:
...
- full content list cuối cùng của mọi skill/relic.
...
- dice order / sequencing là sức mạnh,
