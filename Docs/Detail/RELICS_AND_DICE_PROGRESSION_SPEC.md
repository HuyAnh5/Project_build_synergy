...
- bẻ hướng run,
- tạo tăng trưởng không chỉ nằm ở skill,
...
- cho player nhặt nhiều relic nhỏ kiểu STS-like thay vì chỉ có 1 relic slot lớn.
...
Pool current direction chia consumable thành 3 nhóm:
...
#### A. Fate
Dùng để can thiệp vào dice / roll / used state, ví dụ:

- reroll dice,
- reload / refresh dice đã used,
- chọn mặt muốn,
- chỉnh value mặt dice,
- copy / paste face,
- enchant face,
- sculpt dice dài hạn.

Tên Fate được dùng vì nhóm này không chỉ “forge” dice, mà còn bẻ kết quả, reload và reroll trong combat.
...
- gây damage,
- áp / khai thác status,
- cứu nguy trực tiếp trong combat,
- bẻ board state theo cách trực tiếp.
...
- hỗ trợ player mà không chiếm action slot của skill.

### 3.3 Không có rarity kiểu common / uncommon / rare nội bộ consumable

Consumable trong hệ này **không có rarity nội bộ kiểu truyền thống**.  
Reward screen có thể có rarity của card reward, nhưng bản thân Fate / Seal / Rune không cần phân tầng common/uncommon/rare riêng.

### 3.4 Không có unlock riêng cho consumable

Các consumable này **không cần unlock** trong scope hiện tại. Chúng tồn tại trong game ngay từ đầu; khác nhau chỉ ở chỗ player có gặp và nhặt được trong run hay không.
...
Player được chỉnh dice và bẻ turn **thường xuyên**, không phải hiếm hoi.
...
`Run tiếp tục`  
→ Seal / Rune cho utility hoặc power spike trực tiếp  
→ Fate thay đổi chính bộ dice  
...
Mỗi run không chỉ cho player nhặt skill/relic tốt hơn.  
...
## 6. Fate — dice / roll / reload consumables

### 6.1 Vai trò

Fate là nhóm consumable dùng để can thiệp vào dice, roll, face và used state. Chúng có thể được dùng:

- **trong combat** để cứu roll xấu, reroll, reload dice đã used, chọn mặt hoặc bẻ turn hiện tại,
- **trong shop / loadout overlay** để sculpt build dài hạn và thao tác trên nhiều dice cùng lúc.

Rule rất quan trọng:

- flow dùng Fate không khóa cứng thứ tự chọn,
...
- cùng một nhóm Fate có thể tồn tại cả:
...
### 6.2 Pool Fate hiện tại

Các effect Fate hiện tại gồm các nhóm:
...
- Guard Boost
- Gold Proc
- Fire / Ice / Bleed / Lightning enchant
- Reroll dice
- Reload / refresh dice đã used
- Chọn mặt muốn nếu consumable effect cho phép

### 6.3 Fate targeting / overwrite rules

- Fate chỉnh mặt phải ghi rõ chọn bao nhiêu face, trên bao nhiêu dice.
- Enchant Fate gắn lên face theo target requirement của effect.
- Enchant mới ghi đè enchant cũ trên chính mặt đó nếu cùng slot enchant.
- Value +N là Added Value modifier, không tạo effect riêng nếu text không ghi.

### 6.4 Flow tương tác hiện tại cho Fate / dice edit
...
4. Khi player bấm **Use**, hệ thống mở dice-edit overlay theo đúng context hiện tại nếu effect cần overlay.
5. Trong overlay:
   - player xoay dice đang được hiển thị,
   - chọn đủ mặt theo target requirement của effect,
...
- **Combat context**: thường hiển thị / thao tác 1 dice tại một thời điểm nếu effect yêu cầu.
- **Shop / loadout overlay context**: có thể hiển thị cả 3 dice, nên các effect có thể target mặt ở nhiều dice khác nhau nếu text cho phép.
...
**Lightning**
- mặt này được tính là **cả Crit và Fail** cho các điều kiện skill / relic
...
- rewrite cách mặt đó được đọc cho condition của skill / relic:
  - luôn tính là `Crit`
  - luôn tính là `Fail`
...
## 9. Seal — direct combat consumables
...
## 10. Rune — buff / utility consumables

Rune là nhóm consumable dùng trong combat để:
...
Các biến thể này chỉ nên được giữ nếu chúng không làm trùng vai quá mạnh với Seal named pool ở trên.
...
### 12.2 Không để consumable thay hết skill / relic

Consumable phải là công cụ bẻ nhịp hoặc sculpt run,  
không được nuốt luôn vai trò của skill / relic hoặc phá combat loop nền.
...
## 12A. Relic collection — current direction

Relic là tên mới cho lớp relic-effect của run.

Current direction:

- player có thể nhặt **nhiều relic** trong một run,
- relic hiển thị ở góc trên UI giống STS-like relic bar,
- relic effect nhỏ / hẹp hơn passive cũ vì chúng có thể cộng dồn,
- không còn 1 relic slot duy nhất,
- reward / shop / event có thể cho relic như một loại build-shaping reward.

Guardrail:

- relic không nên mạnh như passive cũ nếu player có thể nhặt nhiều,
- relic nên tạo hướng build, utility hoặc synergy nhỏ,
- sức mạnh lớn nên đến từ tổ hợp skill + dice + relic + consumable, không phải 1 relic đơn lẻ.
...
- consumable chia thành **Fate / Seal / Rune**,
...
- flow dùng Fate / dice-edit hiện tại là **state-based selection**,
...
- trong combat, player có thể edit dice trong **Player Phase** nếu consumable cho phép,
...
- không phải relic áp lên cả build
...
- Fate / dice-edit consumable flow thật vẫn chưa khóa
...
3. nối `selected logical face` vào Fate consumable đầu tiên
4. phân biệt rõ:
...
- chưa có `consumable data model` thật để author `Fate / Seal / Rune`
- chưa có runtime inventory cho `3 shared consumable slot`
- chưa có `use flow` đầy đủ cho consumable trong combat
- chưa có `use flow` đầy đủ cho Fate dice-edit ngoài combat / trong sandbox
- chưa có link giữa `selected logical face` và một consumable Fate cụ thể
...
3. `lớp tương tác / sử dụng`
   ...
   - chưa có route riêng cho:
     ...
     - Fate dùng để edit dice,
     - consumable utility như heal / AP / cleanse / double gold / cheat death.
...
3. nối `use flow`
4. rồi mới nối consumable Fate đầu tiên vào dice-edit sandbox / combat flow

Thứ tự này quan trọng vì:

- nếu làm `slot/use flow` trước khi có data model, hệ sẽ dễ vỡ,
- nếu nối Fate vào sandbox trước khi có shared-slot runtime, sẽ rất dễ phải viết tạm logic riêng rồi đập đi,
