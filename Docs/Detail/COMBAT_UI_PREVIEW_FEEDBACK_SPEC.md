...
Game có nhiều layer như dice, Focus, Guard, Stagger, status, passive, buff/debuff và modifier. Player không nên phải tự tính tất cả trong đầu. UI phải cho họ thấy:

- skill sẽ dùng bao nhiêu Focus;
...
Focus/Mana hiển thị bằng:

- con số tổng hiện tại;
- các đoạn/segment nhỏ;
- mỗi segment tượng trưng 1 Focus/Mana.

Hướng hình ảnh giống Expedition 33:

```text
Number + segmented bar
```

Khi hover hoặc drag skill:

- Focus/Mana cost thật của skill được preview;
- các segment sẽ bị tiêu chuyển vàng;
- các segment đó nhấp nháy nhẹ;
- số Focus/Mana cũng preview thành số còn lại sau khi trừ.

Resource preview phải là cost thật sau mọi modifier.

Ví dụ:

```text
Skill cost gốc = 2 Focus.
Nếu có modifier làm skill +1 Focus cost,
hover phải preview cost = 3 Focus.
```

Không được hover hiện 2 nhưng cast thật trừ 3.

Nếu không đủ Focus/Mana:

- vẫn preview phần cost;
- phần resource preview nhấp nháy;
- background Focus/Mana chuyển đỏ;
...
Nếu skill thiếu Focus/Mana hoặc thiếu dice:
...
Player có passive giả định +20% damage output.
Enemy đang Stagger.
```
...
```text
Skill cost gốc = 2 Focus.
Modifier khiến mọi skill +1 Focus cost.
Hover preview phải trừ 3 Focus.
```
...
Dice có 3 trạng thái roll chính:

### Normal

- giữ visual như hiện tại.

### Crit

- dice có outline màu vàng.

### Fail

- dice có outline đỏ nhẹ.

Crit / Fail phải đọc được ngay sau roll.

Nếu dice đang được preview consume, trạng thái consume preview có thể chồng lên Crit/Fail nhưng không được làm mất thông tin Crit/Fail.
...
Hiệu ứng dừng lần lượt chỉ áp dụng cho Roll Phase đầu lượt.
...
- thiếu Focus/Mana → resource background đỏ;
...
- hover skill preview đúng Focus/Mana cost thật;
