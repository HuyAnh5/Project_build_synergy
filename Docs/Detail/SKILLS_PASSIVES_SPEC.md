...
> Tài liệu này là **source of truth cho content layer hiện tại của skill, relic, combo engines và triết lý thiết kế content**.
...
> Exact tuning numbers có thể sống trong game data / implementation nếu còn volatile. File này ưu tiên rule direction, wording, build role và constraint; không tự biến mọi số trong docs thành balance final nếu chưa được khóa.
...
* làm chuẩn cho việc thêm skill/relic mới sau này,
...
### 2.0A Current content rule — blue value và elemental payoff

- Số thường không scale theo dice / Added Value / Crit / Fail.
- Chỉ số được đánh dấu là **blue value** mới nhận Added Value và modifier hợp lệ.
- Element payoff là skill-text driven: skill nào consume Burn, khai thác Freeze/Chilled, phá Mark hoặc spread Mark phải ghi rõ.
- Ở scope hiện tại, mỗi hệ chủ yếu tương tác với status của chính hệ đó. Cross-element reaction chưa nằm trong locked rule.
- Relic thay cho passive cũ ở tầng progression: player có thể nhặt nhiều relic, nên từng relic phải nhỏ/hẹp hơn passive cũ.
...
* nhưng khi đặt cạnh đúng relic / đúng status / đúng dice engine thì mở ra lớp giá trị mới,
...
### 2.3A Element / tooltip / trigger rule

Tooltip của skill phải hiển thị rõ:

* `Element`
* `Tag`
* output nào là **blue value**
...
* Skill **không tự kích hoạt payoff của hệ** nếu text skill không ghi payoff đó.
* Skill chỉ `apply Burn`, `apply Freeze`, `apply Mark`, buff, debuff, hoặc gain Guard thì chỉ là setup trừ khi text ghi thêm payoff.
* Direct damage hit chỉ là một điều kiện mà skill có thể đọc; nó không tự tạo payoff nền.

Rule theo hệ hiện tại:

* `Fire`: skill nào consume Burn phải ghi rõ. Damage/effect khi consume Burn do chính skill quyết định.
* `Ice`: skill nào khai thác Freeze/Chilled để nhận AP, Guard, kéo dài control hoặc tạo payoff khác phải ghi rõ.
* `Lightning / Mark`: skill nào phá Mark, giữ Mark, spread Mark hoặc tạo shock/payoff từ Mark phải ghi rõ.

Không còn baseline kiểu “mọi Fire hit tự consume Burn”, “mọi Ice hit tự +Guard/AP”, hoặc “mọi Lightning hit tự proc Mark”.
...
* Gây damage theo blue value `"X" = Base Value + Added Value`
* Gây damage chuẩn với blue fixed/base output rồi cộng Added Value
* Gây damage AoE
* Gây bonus damage theo condition

Rule chung từ current direction:

* Chỉ output được đánh dấu blue value bằng dấu `"..."` mới cộng Added Value.
* Số fixed/scalar không có dấu `"..."` không tự cộng Added Value.
* Rule này không chỉ áp cho `Attack`; `Guard`, status output và history payoff số học cũng đi cùng logic nếu chúng được đánh dấu blue value.

#### B. Guard Modules

* Nhận Guard theo giá trị dice
* Nhận Guard cố định nếu là blue value thì cộng Added Value
...
* `Effect / Target State`
* `Resource axis (AP / Guard)`
* `Board / encounter axis`

Ý nghĩa thực dụng:

* game **không thiếu trục**, mà đang hơi dư trục,
* skill/relic mới nên bám rõ `1-2 trục chính`,
...
* **Tag mặc định:** None
* **Slots / AP:** **1 slot, 0 AP**
* **Target:** Single Enemy
* **Range:** Melee
* **Text hiện tại:** Gây `"4 damage"` và hồi 1 AP.
* **Added Value note:** `"4 damage"` là blue value nếu data đánh dấu như vậy, cộng Added Value của die dùng. Basic Attack không phải Physical mặc định.
* **Tag/element override:** Skill hoặc relic có thể đổi tag/element của Basic Attack nếu text ghi rõ.
...
* **Target:** Self
* **Range:** Self
* **Text hiện tại:** Nhận Guard theo data hiện tại của Basic Guard.
* **Tag/element override:** Skill hoặc relic có thể đổi tag/element/cách scale của Basic Guard nếu text ghi rõ.
...
* **Added Value note:** Bản thân skill không có blue value. Basic Attack sau đó vẫn cộng Added Value như Basic Attack bình thường.
...
* **Added Value note:** Không có blue value. Damage theo Burn consume và 7 Burn reapply không cộng Added Value.
* **Vai trò thiết kế:** win condition exact-value cực sâu của Fire build.
* **Role trong build:** Core Piece
* **Note cực quan trọng:** anti-synergy cứng với relic làm thay đổi mặt dice như `Crit Escalation` là **intentional**.
* **Trạng thái:** **Đã chốt rõ**
...
* **Added Value note:** Không có blue value.
...
* **Element:** Physical
* **Tag:** `Attack`
* **Slots / AP:** **1 slot, 1 AP**
* **Rarity:** pending
* **Target:** Single Enemy
* **Range:** Melee
* **Condition chính:** Target đã có Mark trước khi trúng
* **Effect Modules:** Fixed Damage + AP Refund
* **Text hiện tại:** Gây `"6 damage"`. Nếu mục tiêu đã có Mark trước khi trúng, hồi ngay 1 AP.
...
* **Added Value note:** `"X damage"` lấy Base cao nhất, rồi cộng Added Value của cả 2 dice trong local group.
* **Ví dụ:** roll 5 và 10, cả hai đều Crit -> lấy Base 10 và cộng Added Value của cả 5 lẫn 10.
...
* **Element:** Physical
* **Tag:** `Attack`
* **Slots / AP:** **1 slot, 3 AP**
* **Rarity:** Rare
* **Target:** Single Enemy
* **Range:** Melee
* **Condition chính:** Base Value = 1
* **Effect Modules:** Exact-value payoff + High fixed damage
* **Text hiện tại:** Nếu Base Value = 1, skill tốn 3 AP và gây `"12 damage"`.
...
* **Element:** pending
* **Tag:** `Attack`
* **Slots / AP:** `Adaptive slot`, AP pending
* **Rarity:** pending
* **Target:** Single Enemy
* **Range:** Pending
...
* **Vai trò:** Bleed setup signature; crit vào đúng die sẽ mở bùng nổ stack rất rõ.
...
| Relic                          | Rarity       | Text hiện tại                                                                                                          | Vai trò thiết kế                                                                                                            | Trạng thái          |
| -------------------------------- | ------------ | ---------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- | ------------------- |
...
| **Clear Mind**                   | **Uncommon** | Bắt đầu mỗi lượt, hồi thêm 1 AP.                                                                                    | Economy engine universal, giúp xoay lượt và làm cầu nối cho nhiều build                                                     | **Revised đã chốt** |
...
* Chỉ output được đánh dấu **blue value** mới cộng `Added Value`.
* Trong file này, blue value được viết bằng dấu `"..."`, ví dụ `"6 damage"`, `"X Burn"`, `"X Guard"`.
* Số không có dấu `"..."` là rule/fixed/scalar và không tự cộng Added Value.
* `X` trong blue value mặc định là **`Base Value + Added Value`**.
* `Basic Attack` có `"4 damage"` nên cộng Added Value nếu có.
* `Basic Guard` có `"Guard"` nên cộng Added Value nếu có.
...
* Về sau có thể tồn tại skill hoặc relic cho thêm Added Value nếu đạt điều kiện, kể cả theo từng skill riêng lẻ hoặc tăng tiến trong combat/run.
* Nếu skill chiếm 2 hoặc 3 slot và chỉ có **1 blue output**, Added Value mặc định là **tổng Added Value của toàn bộ dice trong local group** của skill đó.
* Dice có thể mang / sinh Added Value từ **Crit** hoặc từ **enchant / consumable / dice customization** đã nằm trên mặt dice; ngoài ra Added Value cũng có thể do skill/relic/modifier cấp.
...
* `Blue damage chuẩn` -> `Final Damage = damage gốc + Total Added Value`
* `Blue X` -> `X = Base Value + Added Value`
* `Blue fixed Guard / status / history output` -> `Final Output = output gốc + Total Added Value`
* `Non-blue number` -> không cộng Added Value
...
* `Damage từ combat history counter` chỉ cộng `Added Value` nếu output đó là blue value hoặc text ghi rõ
...
* `Clear Mind` tăng nhịp hồi AP mỗi lượt.
* Kết hợp với `Hellfire` có thể tạo loop gần hoặc hoàn toàn tự duy trì.
* Đây là interaction mạnh nhưng là loại sức mạnh phải đánh đổi relic flexibility.
...
Nhiều relic và một phần skill hiện tại cố tình làm Basic Attack hoặc Basic Guard có giá trị build-level.
Đây là định hướng cần giữ, không phải filler tạm thời.

### 5.5 Build thuần mạnh hơn, nhưng utility ngoài hệ vẫn hợp lệ

Game cho phép giữ 1–2 skill hoặc relic trái hệ / universal để vá nhịp, giữ sống sót hoặc nối engine.
...
Build này mạnh nhất khi combat có **3 kẻ địch**, nhưng vẫn cần còn giá trị ở combat ít địch hơn nhờ nhánh Guard / AP / Chilled payoff.
...
```text
Dice Forging -> tăng base mặt trọng yếu -> relic đọc parity/exact value -> skill payoff bùng nổ
```
...
## 7. Guardrail khi thêm skill/relic mới
...
* `AP hiện tại` hay `AP còn lại sau cost`
...
* skill hiện tại như trên,
* 11 relic hiện tại như trên,
...
- Relic: Clear Mind, Even Resonance, Elemental Catalyst, Fail Forward, Iron Stance, Crit Escalation, Dice Forging
...
- hardcode skill/relic theo display name
...
- migrate thêm skill/relic còn lại vào runtime theo hướng trên
