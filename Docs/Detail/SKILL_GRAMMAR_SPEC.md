...
- tạo chung một “grammar” để thiết kế skill, relic và tooltips,
...
### 3.2 Added Value và Blue Value

**Added Value** là phần cộng thêm vào output cuối. Added Value **không đổi bản chất của Base Value**.

Current locked direction:

- **Số thường** = fixed / rule number, không bị dice, Crit, Fail, Added Value, vị trí hoặc modifier tác động.
- **Số xanh / blue value** = output được phép nhận Added Value và modifier hợp lệ.
- Skill chỉ cộng Added Value vào output được đánh dấu là blue value.
- Không còn rule “mọi output số học mặc định cộng Added Value”.
- Không còn rule “nếu skill có nhiều output, Added Value mặc định cộng vào output đầu tiên”.
...
`4` là số thường nếu không được đánh dấu blue value.

```text
Deal "4 damage".
```

`"4"` là blue value và có thể nhận Added Value.

Blue value có thể là damage, Guard, Burn, Bleed, AP, heal hoặc output số học khác nếu skill data đánh dấu nó là blue.

Ví dụ:
- Base Value = 4, Added Value = +3  
→ die vẫn được condition đọc là **4**, không phải 7.
...
- Skill / relic / combat modifier có thể cấp Added Value nếu text ghi rõ
- Fail không phải nguồn Added Value âm

### 3.3A Skill Formula Sheet
...
- `Số thường` = fixed number, không scale.
- `Blue value` = output được phép nhận Added Value / modifier hợp lệ.
- `Crit`:
  - non-Physical -> `Added Value += floor(Base Value x 0.2)` nếu action/output đang nhận crit scaling
  - Physical -> `Added Value += floor(Base Value x 0.5)` nếu action/output đang nhận Physical crit scaling
- `Fail` -> penalty / modifier lên output hợp lệ theo rule của skill; Fail không phải Added Value âm.
- Skill nhiều slot cộng Added Value của các dice trong local group **chỉ vào blue output hợp lệ**.
- Split-role skill: mỗi branch chỉ đọc Base/Added/Crit/Fail của đúng die source mà branch đó chọn.
- Toàn game dùng `floor`.
- Nếu `base output > 0` nhưng sau Fail / floor tụt xuống dưới `1`, final output vẫn phải tối thiểu là `1` trừ khi skill ghi rõ khác.
- Nếu `base output = 0`, kết quả tiếp tục là `0`.
...
- `highest -> Mark spread`, `lowest -> self AP`
...
- Ranged
- Self
- Pending / special case n §¨u skill cAýn ch’øa ch ¯`t
...
### 9.0A Rule cho condition branch

- Nếu condition chỉ cấp Added Value, refund, AP gain, Guard gain, hoặc một modifier dùng để chỉnh output chính, thì phần này resolve ngay trong nửa đầu của skill.
- Nếu condition tạo ra một effect combat riêng như deal damage, apply status/effect lên enemy, consume status để payoff, thì phần này không resolve chèn vào nửa đầu.
- Coi nó như follow-up step: base hit / base output resolve trước, rồi branch damage/status/consume theo condition resolve sau.
- Tóm gọn:
  - Condition = Added Value / modifier -> resolve ngay.
  - Condition = damage / status / consume / payoff effect riêng -> resolve nửa sau.
...
- **Note:** anti-synergy có chủ đích với relic tăng mặt dice
...
- **Element:** Physical
- **Tag:** `Attack`
- **Slots / AP:** `1 slot, 4 AP`
- **Rarity:** `Rare`
- **Target:** Single Enemy
- **Range:** Melee
- **Condition chính:** `Base Value = 1`
- **Condition Scope:** Die Scope
- **Effect Modules:** High fixed damage + AP Cost Override
- **Text mẫu:** Tốn 4 AP. Nếu Base Value = 1, skill này chỉ tốn 2 AP và gây 20 damage.
...
- `Target + Range + 1 condition chính + 1–2 module chính` là công thức mặc định,
...
## 15. Progress note (Archived)

Đoạn này ghi lại hướng runtime/authoring đang được áp dụng.

### 15.1 Hiện tại grammar đang được đưa vào runtime theo cách nào

Project đang đi theo lớp:

- engine code
- content data
- custom behavior cho trường hợp exception

Đây là bước chuyển tiếp giữa:

- hardcode theo tên skill
- và một hệ module data-driven đầy đủ hơn trong tương lai

### 15.2 Behavior id là bước trung gian đang dùng

Hiện tại một số skill/relic đã có `behaviorId` để runtime nhận ra mechanic identity mà không cần string match theo tên.

Ý nghĩa:

- inspector/asset bắt đầu trở thành source of truth cho "skill này là mechanic nào"
- runtime vẫn resolve bằng code
- các skill đặc thù vẫn được cho phép có custom hook

### 15.3 Hướng grammar tiếp theo nên là gì

Cần mở rộng dần thêm:
...
- phần lớn skill/relic thường có thể lắp từ grammar/module
- skill/relic hiếm hoặc quá đặc biệt mới cần custom resolver

### 15.4 Guardrail

Không nên cố gắng ép 100% mọi skill/relic vào inspector nếu điều đó biến inspector thành một ngôn ngữ lập trình tệ hơn code.

Mục tiêu tốt hơn là:

- đa số content làm bằng data
- engine và resolver nằm trong code
- exception có hook rõ ràng, có tên rõ ràng
