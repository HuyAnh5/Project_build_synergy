...
- Nếu condition chỉ cấp Added Value, refund, Focus gain, Guard gain, hoặc một modifier dùng để chỉnh output chính, thì phần này resolve ngay trong nửa đầu của skill.
...
Hiện tại một số skill/passive đã có `behaviorId` để runtime nhận ra mechanic identity mà không cần string match theo tên.
...
- phần lớn skill/passive thường có thể lắp từ grammar/module
- skill/passive hiếm hoặc quá đặc biệt mới cần custom resolver

### 15.4 Guardrail

Không nên cố gắng ép 100% mọi skill/passive vào inspector nếu điều đó biến inspector thành một ngôn ngữ lập trình tệ hơn code.
