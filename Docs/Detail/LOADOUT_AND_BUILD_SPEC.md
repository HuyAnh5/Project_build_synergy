...
> Tài liệu này mô tả **cấu trúc build**, **loadout system**, vai trò của **Basic Attack / Basic Guard**, quan hệ giữa **skill / relic / dice**, và cách game muốn các build khác nhau thực sự **chơi khác nhau**.
...
> **Playtest risk:** Dice reorder là core interaction, nhưng 1–3 dice không tự tạo chiều sâu. Skill/relic/enemy content phải tạo tension quanh dice order, dice value, slot cost, timing, high/low, even/odd, crit/fail và multi-dice consumption.
...
→ content mới thuộc một trong các loại: skill / dice / relic / consumable
...
→ nếu slot hữu hạn của loại đó đã đầy, player phải bỏ hoặc bán món cũ mới lấy được món mới; riêng relic hiện là collection nhặt nhiều, không phải 1 slot duy nhất

### 2.2 Flow loadout trước combat

`Chuẩn bị vào combat`
→ hệ thống đọc đúng **4 skill slot**, **relic collection**, **1–3 dice đang equip**, cùng với **Basic Attack / Basic Guard**
...
* **4 skill slot**
* **Relic collection**: có thể nhặt nhiều relic trong run, hiển thị dạng icon ở góc trên UI như STS-like relic bar
* **1 đến 3 dice slot**
...
* nhưng mỗi skill có thể có dice cost / slot cost **1-2-3** khi cast,
* skill 3-dice không chiếm 3 chỗ trong loadout; nó chỉ ăn 3 dice/action economy trong turn đó,
* Relic không còn là 1 relic slot duy nhất; player có thể nhặt nhiều relic,
* vì có thể nhặt nhiều, từng relic phải nhỏ / hẹp hơn passive cũ và không nên một mình định đoạt toàn bộ run.
...
Là lớp relic-effect tích lũy trong run. Relic quyết định build **đọc dice**, **đọc economy**, **đọc trạng thái** và **đọc basic action** như thế nào.

Relic hiện theo hướng STS-like:

- nhặt được nhiều,
- hiển thị icon ở góc trên,
- hiệu ứng từng relic nhỏ hơn passive cũ,
- sức mạnh đến từ tổ hợp nhiều relic + skill + dice, không phải 1 relic slot quá mạnh.

#### Dice slots

Là lớp quyết định **action economy** và **identity của từng turn**. Số dice equip gần như tương ứng với số action groups player có thể dùng trong một lượt.
...
Skill, relic và dice đều có thể được thay đổi ngoài combat.
...
* **0 AP**
* gây blue value **`"4 damage"`** nếu data đánh dấu nó là blue value
* cho **+1 AP**

Basic Attack không chỉ là fallback. Nó còn là điểm gắn với:

* economy loop,
* low-risk sequencing,
* các relic / skill payoff liên quan hit cơ bản,
* những build muốn dùng 1 slot để vá nhịp thay vì commit quá sâu.

Skill hoặc relic có thể đổi tag/element/cách scale của Basic Attack nếu text ghi rõ.
...
* **Element mặc định:** `None`
* **0 AP**
* tạo Guard theo data hiện tại của Basic Guard

Basic Guard không chỉ là nút thủ. Nó là nền cho:
...
* nhưng player vẫn được phép giữ **1–2 skill hoặc relic trái hệ / universal** để vá nhịp, giữ sống sót, hoặc làm cầu nối tạm thời.

Ví dụ các món off-build hợp lệ:

* relic kiểu `mỗi turn +1 AP`,
...
5. thêm skill / dice / relic / consumable đẩy đúng engine,
...
**Loop ngắn:** turn đầu cố Freeze sớm một mục tiêu khi combat có 3 địch để giảm áp lực xuống còn 2 địch hoạt động → trong cửa sổ control đó, dùng các skill băng khác để tăng Guard và khai thác payoff theo Guard → giữ nhịp sống sót, AP và tempo → tiếp tục kiểm soát combat dài hơi.
...
**Lưu ý:** build này mạnh nhất khi đúng điều kiện, nhưng vẫn phải còn giá trị ở combat ít địch hơn thông qua lớp Guard / AP payoff.
...
### 13.1 Mỗi skill / relic mới nên trả lời được
...
* unlock order chi tiết của từng skill / relic / dice,
...
* **4 skill slot / relic collection / 1–3 dice slot**,
* **Basic Attack** và **Basic Guard** luôn tồn tại,
* không có inventory dự trữ,
* full slot thì phải thay món cũ hoặc bỏ qua,
* skill / relic / dice đều được swap ngoài combat,
* build identity phải đến từ **skill + relic + dice + sequencing**,
